...
- es5 变量提升不合理，不支持块级作用域，有关系
    ...
    - 比如面向对象，是最重要的软件思想，设计起来比较复杂（class，constructor，super，extends，public，private），大写的
    函数（constructor）+ prototype
    - 其他的语言都要支持块级作用域，而es5 不用支持，为了简单设计
...
- 继续执行到块级作用域
    ...
    - 在词法环境内部，维护了一个小型栈结构
        块级作用域执行时，首先会找到块级作用域的里面的变量
            词法环境中找变量，在栈顶查找
        每个块级作用域执行完后，要出栈，可以确保外界不可访问
