...
- mockjs 流式输出
  ...
  - 前端 事件监听 SSE：Server-Sent Events 流式响应  @ai/sdk 封装了
  - 响应头设置
...
- 封装了 useDebounce 防抖 hooks
  - 响应式的 debounce 值
  - useEffect 清除函数 来实现防抖功能
  - 专一功能

### 语义搜索 embedding

例如 hello 和 你好 意思是一样的  文本匹配会失效，通过语义相似度来匹配
转换为数学 向量的概念 高纬度  openai: 1536维
- openai embedding.create() 接口
- fs/promises 文件模块 读取文件
  - readFile
    JSON.parse
  - writeFile
    JSON.stringify()
