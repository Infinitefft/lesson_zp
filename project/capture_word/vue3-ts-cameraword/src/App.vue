...
const userPrompt = `
  分析图片内容，找出最能描述图片的一个英文单词，尽量选择更简单的 A1 ~ A2 的词汇。
...
const update = async (imageData: string) => {
  ...
  sentence.value = replyData.explaination;
  explainations.value = replyData.explaination.split('\n').filter((item: string) => item !== '');
  expReply.value = replyData.explaintion_replys;
...
<template>
  <div class="container">
    <PictureCard :word="word" :audio="audio"
      @updata-image="submit"
    />
    <div class="output">
      <div>{{ sentence }}</div>
